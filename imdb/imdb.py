...
def imdb(ctx):
    ...
    conn = psycopg2.connect(dbname='imdb', user='imdb',
                        password='some_password', host = 'localhost', port = '5432')
...
def load_imdb(ctx):
    ...
    with conn.cursor() as cursor:
        for data_file in Path(settings.get('data')).glob('*.tsv'):
            print(data_file)
            table = data_file.system
            print(table)
            sql_statement = f"copy raw.{table} from stdin with tsv header delimiter as ';'"
            print(sql_statement)
            buffer = io.StringIO()
